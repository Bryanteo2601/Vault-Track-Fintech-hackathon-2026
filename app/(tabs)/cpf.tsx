...
export default function CPFScreen() {
  ...
  // CPF data - empty for new users
  const cpfData = {
    oa: { balance: 0, accountType: 'OA' as const },
    sa: { balance: 0, accountType: 'SA' as const },
    ma: { balance: 0, accountType: 'MA' as const },
  };

  const totalCPF = cpfData.oa.balance + cpfData.sa.balance + cpfData.ma.balance;
  const hasCPFData = totalCPF > 0;
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingVertical: 16 }} showsVerticalScrollIndicator={false}>
        {!hasCPFData ? (
          <View style={{ alignItems: 'center', justifyContent: 'center', paddingVertical: 60 }}>
            ...
            <Text style={{ fontSize: 16, fontWeight: '600', color: colors.foreground, marginBottom: 8, textAlign: 'center' }}>No CPF Data Yet</Text>
            <Text style={{ fontSize: 13, color: colors.muted, textAlign: 'center', paddingHorizontal: 20 }}>Your CPF account information will appear here once linked.</Text>
          </View>
        ) : (
          <>
            {/* Total CPF Balance */}
